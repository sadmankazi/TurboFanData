...
# Feature selection
data_analysis= data_train1.describe().T
data_analysis['RUL Corr'] = data_train1.corr()['RUL']
print(data_analysis)

# It doesn't appear these columns do much, so drop:
data_train1 = data_train1.drop([ 'sensor22', 'sensor23', 'sensor25', 'sensor26', 'sensor24', 'Unit', 'sensor7','sensor8','sensor2','sensor6',
                       'opset1', 'opset2', 'opset3', 'sensor1', 'sensor5', 'sensor10', 'sensor18', 'sensor16', 'sensor19', 'sensor20',
                       'sensor15', 'sensor13','sensor14','sensor20'], axis=1)
...
plt.plot(data_train1['RUL'], data_train1['sensor4'], 'o', alpha=0.3)
plt.show()

plt.plot(data_train1['RUL'], data_train1['sensor12'], 'o', alpha=0.3)
plt.show()
...
X_train, X_test, y_train, y_test = train_test_split(data_train1.drop(['RUL'],axis=1), data_train1["RUL"], test_size=0.25, random_state=5)
...
print('RMSE from LR= {}'.format(round(mse, 3)))
...
# print(df.head())
X_train, X_test, y_train, y_test = train_test_split(data_train1.drop(['RUL','RUL_passfail'], axis=1), data_train1['RUL_passfail'], test_size=0.25, random_state=5)
...
print('Accuracy= {}'.format(round(cv_results.mean(), 3)))
